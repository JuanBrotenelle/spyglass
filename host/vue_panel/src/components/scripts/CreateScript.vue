...
<template>
  <div
    class="absolute z-[999999] flex items-center justify-center top-0 left-0 w-full h-screen"
  >
    <div class="z-[9999992] bg-white p-5 rounded-lg">
      <div class="flex flex-row items-center justify-between w-full gap-20">
        <p class="text-lg font-medium">Создание скрипта</p>
        <span
          @click="emit('close')"
          class="text-gray-500 cursor-pointer select-none"
          >Закрыть</span
        >
      </div>
      ...
      <button
        @click="handleSubmit"
        class="mt-5 bg-blue-500 text-white w-full rounded-lg p-2 active:scale-[1.02] active:bg-blue-600 transition-all"
      >
        Создать
      </button>
    </div>
    <div
      class="absolute z-[9999991] bg-[#00000033] top-0 left-0 w-full h-screen"
    ></div>
  </div>
</template>
